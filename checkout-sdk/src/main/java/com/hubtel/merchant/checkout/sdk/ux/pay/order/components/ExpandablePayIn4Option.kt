...
internal fun ExpandablePayIn4Option(
    ...
    ExpandablePaymentOption(
        title = stringResource(R.string.checkout_pay_in_four),
        expanded = expanded,
        onExpand = onExpand,
        decoration = {
            Row(
        ...
        Column(
            ...
            if (state.repaymentEntries?.isNotEmpty() == true) {
                ...
                if (wallets.isEmpty()) {
                    ...
                    Timber.i("Wallet: ${walletState.accountNo}")
                    if (state.isMomoWalletSelected) {
            ...
            if (state.repaymentEntries?.isNotEmpty() == true) {
                ...
                state.repaymentEntries!!.forEachIndexed { index, entry ->
                    if (index == 0) {
                        Column(
                            modifier = Modifier
                        ...
                        ReviewEntryCard(
                            entry = entry,
                            modifier = Modifier.padding(end = Dimens.paddingNano)
                        )
                        Divider(
                ...
                Text(
                    buildAnnotatedString {
                        ...
                        pop()
                        append(
                            " will be debited in three equal\n" +
                                    "instalments."
                        )
                    },
    ...
    LaunchedEffect(expanded) {
        ...
        if (wallets.isEmpty() && state.repaymentEntries?.isNotEmpty() == true) {
